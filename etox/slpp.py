...
class SLPP:
    ...
    def __encode(self, obj):
        ...
        if obj == None:
            ...
            self.depth += 1
            if len(obj) == 0 or ( tp is not dict and len(filter(lambda x:  type(x) in (int,  float,  long) or (type(x) in [str, unicode] and len(x) < 10),  obj)) == len(obj) ):
                newline = tab = ''
            dp = tab * self.depth
            s += "%s{%s" % (tab * (self.depth - 2), newline)
            if tp is dict:
                s += (',%s' % newline).join([self.__encode(v) if type(k) is int else dp + '%s = %s' % (k, self.__encode(v)) for k, v in obj.iteritems()])
            else:
